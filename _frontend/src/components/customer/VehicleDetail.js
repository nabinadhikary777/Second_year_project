...
const VehicleDetail = () => {
  ...
  const [eligibleBookingForReview, setEligibleBookingForReview] = useState(null);
  ...
  useEffect(() => {
    if (user && vehicle) {
      bookingAPI.getCustomerBookings('all').then((res) => {
        const list = res.data.results || res.data || [];
        const vid = Number(id);
        const forThisVehicle = list.filter((b) => {
          const sameVehicle = Number(b.vehicle) === vid || b.vehicle_details?.id === vid;
          const reviewEligible = ['confirmed', 'ongoing', 'completed'].includes(b.booking_status);
          return sameVehicle && reviewEligible;
        }
        );
        const reviewedIds = new Set(reviews.map((r) => r.booking));
        const unreviewed = forThisVehicle.find((b) => !reviewedIds.has(b.id));
        setEligibleBookingForReview(unreviewed || null);
      }).catch(() => {});
    }
  ...
  const handleSubmitReview = async () => {
    if (!eligibleBookingForReview?.id) {
      toast.error('You can review this vehicle after owner confirmation.');
      return;
    }

    setSubmittingReview(true);
    try {
      await bookingAPI.addReview(eligibleBookingForReview.id, {
        rating,
      ...
      setRating(5);
      fetchVehicleDetails();
      setEligibleBookingForReview(null);
    } catch (error) {
      ...
      <Grid container spacing={3}>
        ...
        <Grid item xs={12} md={8}>
          ...
          <Paper sx={{ p: 3, borderRadius: 2 }}>
            ...
            {/* Rate & Review - Available after owner confirms the booking */}
            {user && eligibleBookingForReview && (
